...
out_filename = 'flux_mock.npy'  # Output filename
...
#@njit(parallel=False, cache=True)
def real_to_redshift_space_old(delta, vel, tweb, ngrid, lbox, rsd_pars) :
...
vz = np.fromfile(open(vz_filename, 'r'), dtype=np.float64)   # In real space
...
print('Computing invariants of tidal and curvature tensors in real space ...')
...
print('Computing invariants of tidal and curvature tensors in redshift space ...')
